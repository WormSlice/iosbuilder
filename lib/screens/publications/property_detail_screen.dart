            ...
            actions: [
              Transform.translate(
                ...
                child: IconButton(
                  ...
                  onPressed: () {
                    final String title = d['title'] ?? 'Inmueble';
                    SharePlus.instance.share(ShareParams(
                      text: '🏠 $title\n📍 $locationText\n\nMira este inmueble en CONNECT \n\nhttps://connectapp.com.co/p/${widget.postId}'
                    ));
                  },
                ),
              ),
            ...
            child: Column(
              ...
              children: [
                  ...
                  children: [
                    SizedBox(
                      ...
                      child: images.isNotEmpty
                          ...
                          : Container(color: Colors.grey[300], child: const Icon(Icons.home_work_outlined, size: 50, color: Colors.grey)),
                    ),
                    if (images.length > 1)
                      Positioned(
                        bottom: 12,
                ...
                Padding(
                  ...
                  child: Text(_getString(d['title'], fallback: 'Propiedad sin título'), style: const TextStyle(fontSize: 22, fontWeight: FontWeight.bold, color: Colors.black, fontFamily: 'CanvaSans')),
                ),
                Padding(
                            ...
                            children: [
                              ...
                              GestureDetector(
                                onTap: () async {
                                  ...
                                  try {
                                    final chatId = await FirestoreService().getOrCreateChat(currentUid, ownerId, publicationId: widget.postId, publicationData: {'id': widget.postId, 'title': d['title'], 'image': images.isNotEmpty ? images.first : null, 'price': d['price'], 'type': 'property'});
                ...
                _buildInfoBox(context, [
                  _buildInfoRow('Tipo de propiedad', _getString(d['propertyType'])),
                  _buildInfoRow('Metraje', '${d['metraje'] ?? 0} m²'),
                  _buildInfoRow('Año construcción', _getString(d['yearBuilt'])),
                  _buildInfoRow('Habitaciones', _getString(d['habitaciones'])),
                  _buildInfoRow('Baños', _getString(d['baños'])),
                  _buildInfoRow('Parqueaderos', _getString(d['parqueaderos'])),
                  _buildInfoRow('Estrato', _getString(d['estrato'])),
                  _buildInfoRow('Antigüedad', _getString(d['antiguedad'])),
                  _buildInfoRow('Tipo de piso', _getString(d['floorType'])),
                ]),
                const SizedBox(height: 8),

                _buildDetailChip('Ubicación', _getString(d['sector'])),
                _buildDetailChip('Permuta', (d['acceptsPermuta'] == true) ? 'Sí' : 'No'),
                _buildDetailChip('Anunciante', _getString(d['announcerType'])),
                    ...
                    children: [
                      ...
                      SizedBox(
                        ...
                        child: FutureBuilder<List<Map<String, dynamic>>>(
                          future: AlgoliaService().getRecommendations(category: d['category'] ?? 'propiedades', currentPostId: widget.postId),
                          builder: (context, snapshot) {
