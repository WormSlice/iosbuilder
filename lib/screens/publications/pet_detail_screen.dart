...
class _PetDetailScreenState extends State<PetDetailScreen> {
  ...
  String _getDisplayPrice(Map<String, dynamic> data) {
    ...
    final type = data['publicationType']?.toString().toLowerCase() ?? data['type']?.toString().toLowerCase() ?? '';

    if (category.contains('masc') || true) {
    ...
    return _formatCop(data['price']);
  }

  String _formatCop(dynamic price) {
    if (price == null) return '\$ 0';
    String raw = price.toString();
    String processed = raw;
    if (raw.contains('.')) processed = raw.split('.').first;
    else if (raw.contains(',')) processed = raw.split(',').first;
            ...
            actions: [
              Transform.translate(
                ...
                child: IconButton(
                  ...
                  onPressed: () {
                    final String title = d['title'] ?? 'Mascota';
                    SharePlus.instance.share(ShareParams(
                      text: '🐾 $title\n📍 $locationText\n\nMira esta publicación de mascota en CONNECT \n\nhttps://connectapp.com.co/p/${widget.postId}'
                    ));
                  },
                ),
              ),
          ...
          SliverToBoxAdapter(
            child: Column(
              ...
              children: [
                  ...
                  children: [
                    SizedBox(
                      ...
                      child: images.isNotEmpty
                          ...
                          : Container(color: Colors.grey[300], child: const Icon(Icons.pets_outlined, size: 50, color: Colors.grey)),
                    ),
                    if (images.length > 1)
                      Positioned(
                        bottom: 12,
                            ...
                            children: [
                              ...
                              GestureDetector(
                                onTap: () async {
                                  ...
                                  try {
                                    final chatId = await FirestoreService().getOrCreateChat(currentUid, ownerId, publicationId: widget.postId, publicationData: {'id': widget.postId, 'title': d['title'], 'image': images.isNotEmpty ? images.first : null, 'price': d['price'], 'type': 'pet'});
                ...
                _buildDetailChip('Tipo de publicación', _getString(d['publicationType'])),
                _buildDetailChip('Especie', _getString(d['especie'])),
                _buildDetailChip('Raza', _getString(d['raza'])),
                _buildDetailChip('Edad', _getString(d['edad'])),
                _buildDetailChip('Vacunas', _getString(d['vacunas'])),
                _buildDetailChip('Sexo', _getString(d['sexo'])),
                _buildDetailChip('Pedigrí', (d['pedigree'] == true) ? 'Sí' : 'No'),

                const SizedBox(height: 8),
                Padding(
                  padding: const EdgeInsets.symmetric(horizontal: 16.0),
                  child: Text(
                    'AVISO LEGAL: CONNECT prohíbe la venta de especies ilegales o en peligro de extinción.',
                    textAlign: TextAlign.center,
                    style: TextStyle(fontSize: 11, fontWeight: FontWeight.w600, color: Colors.red[400], fontFamily: 'CanvaSans'),
                  ),
                ),
                    ...
                    children: [
                      ...
                      SizedBox(
                        ...
                        child: FutureBuilder<List<Map<String, dynamic>>>(
                          future: AlgoliaService().getRecommendations(category: d['category'] ?? 'mascotas', currentPostId: widget.postId),
                          builder: (context, snapshot) {
