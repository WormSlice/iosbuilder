...
class PublishPropertyPanel extends StatefulWidget {
  final String? postId;
  final Map<String, dynamic>? initialData;

  const PublishPropertyPanel({super.key, this.postId, this.initialData});

  @override
  State<PublishPropertyPanel> createState() => _PublishPropertyPanelState();
}

class _PublishPropertyPanelState extends State<PublishPropertyPanel> {
  ...
  String _selectedAnnouncer = 'Propietario';
  ...
  void initState() {
    ...
    if (widget.initialData != null) {
      ...
      _metrajeController.text = (data['metraje'] ?? '').toString();
      _yearController.text = (data['yearBuilt'] ?? '').toString();
      _roomsController.text = (data['habitaciones'] ?? '').toString();
      _bathroomsController.text = (data['baños'] ?? '').toString();
      _parkingController.text = (data['parqueaderos'] ?? '').toString();
      _estratoController.text = (data['estrato'] ?? '').toString();
      _antiguedadController.text = (data['antiguedad'] ?? '').toString();
      _floorTypeController.text = (data['floorType'] ?? '').toString();
      _sectorController.text = (data['sector'] ?? '').toString();

      _selectedPropertyType = _propertyTypes.contains(data['propertyType'])
          ? data['propertyType']
          : _propertyTypes.first;
      _acceptsPermuta = data['acceptsPermuta'] ?? false;
      _selectedAnnouncer = _announcers.contains(data['announcerType'])
          ? data['announcerType']
          : _announcers.first;
  ...
  Future<void> _publish() async {
    ...
    try {
      ...
      final Map<String, dynamic> postData = {
        ...
        'metraje': _metrajeController.text,
        'yearBuilt': _yearController.text,
        'habitaciones': _roomsController.text,
        'baños': _bathroomsController.text,
        'parqueaderos': _parkingController.text,
        'estrato': _estratoController.text,
        'antiguedad': _antiguedadController.text,
        'floorType': _floorTypeController.text,
        'sector': _sectorController.text,
        'acceptsPermuta': _acceptsPermuta,
        'announcerType': _selectedAnnouncer,
        'location': _selectedLocation,
        'latitude': coords?['latitude'] ?? LocationService().selectedLat,
        'longitude': coords?['longitude'] ?? LocationService().selectedLng,
        'type': 'property',
        'category': 'propiedades',
        'subCategory': _selectedPropertyType.toLowerCase(),
        'musicId': _musicId,
        ...
        child: Column(
          ...
          children: [
            ...
            TextField(
              ...
              maxLength: 40,
              decoration: _inputDecoration('Ej: Apartamento en Cedritos'),
            ),
            _buildLabel('Precio'),
            TextField(
              controller: _priceController,
              keyboardType: TextInputType.number,
              decoration: _inputDecoration('Ej: 450000000'),
            ),
            _buildLabel('Descripción (mín. 40 caract.)'),
            TextField(
              controller: _descController,
              maxLines: 3,
              decoration: _inputDecoration('Detalla las características...'),
            ),

            _buildLabel('Tipo de propiedad'),
            _buildDropdown(
            ...
            _buildLabel('Metraje (m²)'),
            TextField(
              controller: _metrajeController,
              keyboardType: TextInputType.number,
              decoration: _inputDecoration('Ej: 85'),
            ),
            _buildLabel('Año de construcción'),
            TextField(
              controller: _yearController,
              keyboardType: TextInputType.number,
              decoration: _inputDecoration('Ej: 2015'),
            ),
            _buildLabel('Habitaciones'),
            TextField(
              controller: _roomsController,
              keyboardType: TextInputType.number,
              decoration: _inputDecoration('Ej: 3'),
            ),
            _buildLabel('Baños'),
            TextField(
            ...
            TextField(
              ...
              keyboardType: TextInputType.number,
              decoration: _inputDecoration('Ej: 4'),
            ),
            _buildLabel('Antigüedad'),
            TextField(
              controller: _antiguedadController,
              decoration: _inputDecoration('Ej: 5 años, Para estrenar...'),
            ),
            _buildLabel('Tipo de piso'),
            TextField(
              controller: _floorTypeController,
              decoration: _inputDecoration('Ej: Madera laminada, Mármol'),
            ),
            _buildLabel('Sector o Barrio'),
            TextField(
              controller: _sectorController,
              decoration: _inputDecoration('Ej: Santa Bárbara, Laureles...'),
            ),

            Row(
              mainAxisAlignment: MainAxisAlignment.spaceBetween,
              children: [
                _buildLabel('¿Acepta Permuta?'),
                Switch(
                  value: _acceptsPermuta,
                  activeThumbColor: const Color(0xFF0094FF),
                  onChanged: (v) => setState(() => _acceptsPermuta = v),
                ),
              ],
            ),
            ...
            _buildDropdown(
              value: _selectedAnnouncer,
              items: _announcers,
              onChanged: (v) => setState(() => _selectedAnnouncer = v!),
            ),
            const SizedBox(height: 15),
