...
class _BarterDetailScreenState extends State<BarterDetailScreen> {
            ...
            actions: [
              Transform.translate(
                ...
                child: IconButton(
                  ...
                  onPressed: () {
                    final String title = d['title'] ?? 'Trueque';
                    SharePlus.instance.share(ShareParams(
                      text: '📝 $title\n📍 $locationText\n\nMira este increíble intercambio en CONNECT \n\nhttps://connectapp.com.co/p/${widget.postId}'
                    ));
                  },
                ),
              ),
            ...
            child: Column(
              ...
              children: [
                  ...
                  children: [
                    SizedBox(
                      ...
                      child: images.isNotEmpty
                          ...
                          : Container(color: Colors.grey[300], child: const Icon(Icons.swap_horiz, size: 50, color: Colors.grey)),
                    ),
                    if (images.length > 1)
                      Positioned(
                        bottom: 12,
                ...
                Padding(
                  ...
                  child: Text(_getString(d['title'], fallback: 'Trueque sin título'), style: const TextStyle(fontSize: 22, fontWeight: FontWeight.bold, color: Colors.black, fontFamily: 'CanvaSans')),
                ),
                Padding(
                            ...
                            children: [
                              ...
                              GestureDetector(
                                onTap: () async {
                                  ...
                                  try {
                                    final chatId = await FirestoreService().getOrCreateChat(currentUid, ownerId, publicationId: widget.postId, publicationData: {'id': widget.postId, 'title': d['title'], 'image': images.isNotEmpty ? images.first : null, 'price': d['price'], 'type': 'barter'});
                ...
                Padding(
                  ...
                  child: Text('Detalles del Trueque', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Colors.grey[800], fontFamily: 'CanvaSans')),
                ),
                const SizedBox(height: 12),

                _buildInfoBox(context, [
                  _buildInfoRow('Ofrece', _getString(d['offering'])),
                  _buildInfoRow('Busca', _getString(d['seeking'])),
                  _buildInfoRow('Modo', _getString(d['exchangeMode'])),
                  _buildInfoRow('Valor Est.', _formatPrice(d['estimatedValue'])),
                  _buildInfoRow('Estado', _getString(d['itemState'])),
                ]),
                const SizedBox(height: 8),

                _buildDetailChip('MODO TRUEQUE', 'SÍ'),
                    ...
                    children: [
                      ...
                      SizedBox(
                        ...
                        child: FutureBuilder<List<Map<String, dynamic>>>(
                          future: AlgoliaService().getRecommendations(category: d['category'] ?? 'trueques', currentPostId: widget.postId),
                          builder: (context, snapshot) {
  ...
  Widget _buildInfoRow(String key, String value) {
    ...
    return Padding(
      ...
      child: Row(
        children: [
          Expanded(flex: 4, child: Text(key, style: TextStyle(fontSize: 14, color: Colors.grey[600], fontFamily: 'CanvaSans'))),
          Expanded(flex: 6, child: Text(value, style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w500, color: Colors.black87, fontFamily: 'CanvaSans'))),
        ],
      ),
    );
  }
