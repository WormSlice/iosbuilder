            ...
            actions: [
              Transform.translate(
                ...
                child: IconButton(
                  ...
                  onPressed: () {
                    final String title = d['title'] ?? 'Servicio';
                    SharePlus.instance.share(ShareParams(
                      text: '🛠️ $title\n📍 $locationText\n\nMira este servicio en CONNECT \n\nhttps://connectapp.com.co/p/${widget.postId}'
                    ));
                  },
                ),
              ),
            ...
            child: Column(
              ...
              children: [
                  ...
                  children: [
                    SizedBox(
                      ...
                      child: images.isNotEmpty
                          ...
                          : Container(color: Colors.grey[300], child: const Icon(Icons.miscellaneous_services, size: 50, color: Colors.grey)),
                    ),
                    if (images.length > 1)
                      Positioned(
                        bottom: 12,
                            ...
                            children: [
                              ...
                              GestureDetector(
                                onTap: () async {
                                  ...
                                  try {
                                    final chatId = await FirestoreService().getOrCreateChat(currentUid, ownerId, publicationId: widget.postId, publicationData: {'id': widget.postId, 'title': d['title'], 'image': images.isNotEmpty ? images.first : null, 'price': d['price'], 'type': 'service'});
                ...
                Padding(
                  ...
                  child: Text('Detalles del Servicio', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Colors.grey[800], fontFamily: 'CanvaSans')),
                ),
                const SizedBox(height: 12),

                _buildInfoBox(context, [
                  _buildInfoRow('Experiencia', _getString(d['experience'])),
                  _buildInfoRow('Método de pago', _getString(d['paymentMethod'])),
                  _buildInfoRow('Tipo de contrato', _getString(d['contractType'])),
                  _buildInfoRow('Modalidad', _getString(d['modality'])),
                  _buildInfoRow('Tiempo de servicio', _getString(d['serviceTime'])),
                  _buildInfoRow('Horario', _getString(d['schedule'])),
                ]),
                const SizedBox(height: 8),

                _buildDetailChip('Categoría', _getString(d['subCategory'])),
                _buildDetailChip('Cobertura', _getString(d['coverage'])),
                _buildDetailChip('Servicio', _getString(d['title'])),
                _buildDetailChip('MODO TRUEQUE', (d['barterMode'] == true) ? 'SÍ' : 'NO'),
                    ...
                    children: [
                      ...
                      SizedBox(
                        ...
                        child: FutureBuilder<List<Map<String, dynamic>>>(
                          future: AlgoliaService().getRecommendations(category: d['category'] ?? 'servicios', currentPostId: widget.postId),
                          builder: (context, snapshot) {
