        ...
        <div className="space-y-12 animate-in slide-in-from-bottom duration-700">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                {!loading && requests.filter(r => filter === 'all' || r.status === filter).map((req) => (
                    <div key={req.id} className="glass-panel border border-zinc-100 p-8 rounded-[2.5rem] shadow-sm hover:border-black transition-all group">
                        <div className="flex justify-between items-start mb-6">
                            <div className="flex items-center gap-3">
                                <div className="w-10 h-10 rounded-xl glass-button flex items-center justify-center text-zinc-500">
                                    <User size={18} />
                                </div>
                                <div>
                            ...
                            <span className={`px-3 py-1 rounded-full text-[8px] font-black uppercase tracking-widest ${req.status === 'pending' ? 'bg-amber-50 text-amber-600' : 'glass-panel-dark text-white'}`}>
                                {req.status}
                            </span>
                        </div>
                        ...
                        <div className="mb-8">
                            ...
                            <p className="text-xs font-bold text-zinc-600 leading-relaxed glass-button p-4 rounded-2xl italic">
                                "{req.note}"
                            </p>
                        </div>
                        ...
                        <div className="flex items-center justify-between pt-6 border-t border-zinc-50">
                            <div className="flex items-center gap-2">
                                <a 
                                    ...
                                    className="p-3 glass-button hover:glass-panel-dark hover:text-white transition-all rounded-xl text-zinc-400"
                                    title="Enviar Email"
                                >
                                    <Mail size={16} />
                                </a>
                            ...
                            <button 
                                ...
                                className="p-3 glass-button hover:bg-red-50 hover:text-red-500 transition-all rounded-xl text-zinc-200"
                            >
                                <Trash size={16} />
                            </button>
                        </div>
            ...
            {!loading && requests.filter(r => filter === 'all' || r.status === filter).length === 0 && (
                <div className="text-center py-20 glass-button rounded-[3.5rem] border border-zinc-100 border-dashed">
