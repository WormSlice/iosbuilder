            ...
            actions: [
              Transform.translate(
                ...
                child: IconButton(
                  ...
                  onPressed: () {
                    final String title = d['title'] ?? 'Alquiler';
                    SharePlus.instance.share(ShareParams(
                      text: '📝 $title\n📍 $locationText\n\nMira este alquiler en CONNECT \n\nhttps://connectapp.com.co/p/${widget.postId}'
                    ));
                  },
                ),
              ),
            ...
            child: Column(
              ...
              children: [
                ...
                Padding(
                  ...
                  child: Text(_getString(d['title'], fallback: 'Alquiler sin título'), style: const TextStyle(fontSize: 22, fontWeight: FontWeight.bold, color: Colors.black, fontFamily: 'CanvaSans')),
                ),
                Padding(
                            ...
                            children: [
                              ...
                              GestureDetector(
                                onTap: () async {
                                  ...
                                  try {
                                    final chatId = await FirestoreService().getOrCreateChat(currentUid, ownerId, publicationId: widget.postId, publicationData: {'id': widget.postId, 'title': d['title'], 'image': images.isNotEmpty ? images.first : null, 'price': d['price'], 'type': 'rental'});
                ...
                _buildInfoBox(context, [
                  _buildInfoRow('Tipo de alquiler', _getString(d['rentalType'])),
                  _buildInfoRow('Depósito', d['hasDeposit'] == true ? 'Sí / ${_formatPrice(d['depositAmount'])}' : 'No'),
                  _buildInfoRow('Estado', _getString(d['itemState'])),
                  _buildInfoRow('Entrega', _getString(d['deliveryMode'])),
                  _buildInfoRow('Devolución', _getString(d['returnMode'])),
                  _buildInfoRow('Penalidad', _getString(d['penaltyForDelay'])),
                ]),
                const SizedBox(height: 8),

                _buildDetailChip('Categoría', _getString(d['subCategory'])),
                _buildDetailChip('Cobertura', _getString(d['location'])),

                const SizedBox(height: 8),

                if (d['rentalRequirements'] != null && d['rentalRequirements'].toString().isNotEmpty) ...[
                  Padding(
                    padding: const EdgeInsets.symmetric(horizontal: 16.0),
                    child: Text('Requisitos para el alquiler', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Colors.grey[800], fontFamily: 'CanvaSans')),
                  ),
                  const SizedBox(height: 8),
                  _buildRequirementsBox(_getString(d['rentalRequirements'])),
                  const SizedBox(height: 12),
                ],
                    ...
                    children: [
                      ...
                      SizedBox(
                        ...
                        child: FutureBuilder<List<Map<String, dynamic>>>(
                          future: AlgoliaService().getRecommendations(category: d['category'] ?? 'alquileres', currentPostId: widget.postId),
                          builder: (context, snapshot) {
