...
class _VehicleDetailScreenState extends State<VehicleDetailScreen> {
            ...
            actions: [
              Transform.translate(
                ...
                child: IconButton(
                  ...
                  onPressed: () {
                    final String title = d['title'] ?? 'Vehículo';
                    SharePlus.instance.share(ShareParams(
                      text: '🚗 $title\n📍 $locationText\n\nMira este vehículo en CONNECT \n\nhttps://connectapp.com.co/p/${widget.postId}'
                    ));
                  },
                ),
              ),
            ...
            child: Column(
              ...
              children: [
                  ...
                  children: [
                    SizedBox(
                      ...
                      child: images.isNotEmpty
                          ...
                          : Container(color: Colors.grey[300], child: const Icon(Icons.directions_car, size: 50, color: Colors.grey)),
                    ),
                    if (images.length > 1)
                      Positioned(
                        bottom: 12,
                ...
                Padding(
                  ...
                  child: Text(_getString(d['title'], fallback: 'Vehículo sin título'), style: const TextStyle(fontSize: 22, fontWeight: FontWeight.bold, color: Colors.black, fontFamily: 'CanvaSans')),
                ),
                Padding(
                            ...
                            children: [
                              ...
                              GestureDetector(
                                onTap: () async {
                                  ...
                                  try {
                                    final chatId = await FirestoreService().getOrCreateChat(currentUid, ownerId, publicationId: widget.postId, publicationData: {'id': widget.postId, 'title': d['title'], 'image': images.isNotEmpty ? images.first : null, 'price': d['price'], 'type': 'vehicle'});
                ...
                Padding(
                  ...
                  child: Text('Detalles del Vehículo', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Colors.grey[800], fontFamily: 'CanvaSans')),
                ),
                const SizedBox(height: 12),

                _buildInfoBox(context, [
                  _buildInfoRow('Marca', _getString(d['brand'])),
                  _buildInfoRow('Modelo', _getString(d['model'])),
                  _buildInfoRow('Año', _getString(d['year'])),
                  _buildInfoRow('Kilometraje', _getString(d['kilometraje'])),
                  _buildInfoRow('Combustible', _getString(d['combustible'])),
                  _buildInfoRow('Color', _getString(d['color'])),
                  _buildInfoRow('Tracción', _getString(d['traccion'])),
                  _buildInfoRow('Capacidad', _getString(d['capacidad'])),
                  _buildInfoRow('Motor', _getString(d['motor'])),
                  _buildInfoRow('Transmisión', _getString(d['transmision'])),
                ]),
                const SizedBox(height: 8),

                _buildDetailChip('SOAT HASTA', _getString(d['soatHasta'])),
                _buildDetailChip('IMPUESTOS', _getString(d['impuestos'])),
                    ...
                    children: [
                      ...
                      SizedBox(
                        ...
                        child: FutureBuilder<List<Map<String, dynamic>>>(
                          future: AlgoliaService().getRecommendations(category: d['category'] ?? 'vehículos', currentPostId: widget.postId),
                          builder: (context, snapshot) {
  ...
  Widget _buildInfoRow(String key, String value) {
    ...
    return Padding(
      ...
      child: Row(
        children: [
          Expanded(flex: 5, child: Text(key, maxLines: 1, overflow: TextOverflow.ellipsis, style: TextStyle(fontSize: 13, color: Colors.grey[600], fontFamily: 'CanvaSans'))),
          const SizedBox(width: 4),
          Expanded(flex: 5, child: Text(value, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w500, color: Colors.black87, fontFamily: 'CanvaSans'))),
        ],
      ),
    );
  }
